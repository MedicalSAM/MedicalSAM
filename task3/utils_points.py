...
def test_single_volume(image, label, net, classes, multimask_output, patch_size=[256, 256], input_size=[224, 224],
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            grid_points = torch.floor(torch.Tensor(grid_points)).int().cuda() 
            inputs_batch=[{'image':inputs[0],'original_size':(imgsize,imgsize),'point_coords':grid_points,'point_labels':point_labels}]
           
            net.eval()
            with torch.no_grad():
                
                outputs = net(inputs_batch, multimask_output, patch_size[0])[0]
                            
                output_masks = outputs['masks']
                out = torch.argmax(torch.softmax(output_masks.float(), dim=1), dim=1).squeeze(0)
        ...
        # only for debug
        if not os.path.exists('/root/autodl-tmp/SAMed1/nxoutput/images/pred'):
            os.makedirs('/root/autodl-tmp/SAMed1/nxoutput/images/pred')
        if not os.path.exists('/root/autodl-tmp/SAMed1/nxoutput/images/label'):
            os.makedirs('/root/autodl-tmp/SAMed1/nxoutput/images/label')
        assert prediction.shape[0] == label.shape[0]
        for i in range(label.shape[0]):
            with open(f'/root/autodl-tmp/SAMed1/nxoutput/images/pred/pred_{i}.txt', 'w') as file:
                for line in prediction[i]:
                    file.write(np.array2string(line))
            with open(f'/root/autodl-tmp/SAMed1/nxoutput/images/label/label_{i}.txt', 'w') as file:
                for line in label[i]:
                    file.write(np.array2string(line))
