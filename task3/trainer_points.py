...
def trainer_synapse(args, model, snapshot_path, multimask_output, low_res):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # prepare grid points
            batch_size, _, imgsize, imgsize=image_batch.shape
            num_points = imgsize // 20
            x_point = np.arange(0, imgsize, 20)
            y_point = np.arange(0, imgsize, 20)
            xv, yv = np.meshgrid(x_point, y_point)
            grid_points = np.column_stack((xv.ravel(), yv.ravel())).astype(int)     # [676,2]
            points_coords = np.tile(grid_points, (1, 1, 1))
            points_coords = torch.floor(torch.Tensor(points_coords)).int().cuda()   # [1,676,2]


            inputs_batch=[]
            for i in range(0,batch_size):
                # prepare labels
                fixlabel = label_batch[i].clone()
                ...
                inputs_batch.append({'image':image_batch[i],
            ...
            for i in range(outputslen):
                ...
                ogi_outputs[i]['masks'] = ogi_outputs[i]['masks'].float().requires_grad_(True)
            ...
            stacked_output1 = torch.stack(output_list1, dim=0).float().requires_grad_(True)
            stacked_output2 = torch.stack(output_list2, dim=0).float().requires_grad_(True)
            stacked_output3 = torch.stack(output_list3, dim=0).float().requires_grad_(True)
            
            outputs={'low_res_logits':stacked_output1.float(),'masks':stacked_output2.float(),'iou_predictions':stacked_output3.float()}
            outputs['low_res_logits'].requires_grad_(True)
            outputs['masks'].requires_grad_(True)
            outputs['iou_predictions'].requires_grad_(True)
        ...
        save_interval = 4 # int(max_epoch/6)
